...
- As **Azure Storage** doesn't support HTTPS infrastructure with custom domains, it was needed to integrate it with [Azure CDN](https://learn.microsoft.com/en-us/azure/cdn/cdn-overview) to enable this functionality. **Note**: it is no longer possible to use the `Standard_Microsoft` SKU for Azure CDN as it was deprecated in [August 2025](https://www.linkedin.com/pulse/azure-cdn-standard-from-microsoft-classic-retirement-what-salamat-l4n1f/). The only options within Azure is to use [Azure Front Door](https://learn.microsoft.com/en-us/azure/frontdoor/front-door-overview) on its [standard or premium tier](https://learn.microsoft.com/en-us/azure/frontdoor/front-door-cdn-comparison) which could be cost-prohibited if you only plan to host a site on that service (that's why I'm using an **Azure CDN** instance which I had deployed before deprecation for this project).
...
- As of July 15, 2026 new GitHub repositories default to [immutable subject claims](https://docs.github.com/en/actions/reference/security/oidc#immutable-subject-claims) to protect from recycled namespaces (where a different owner could potentially spoof a subject value). For repositories created before that date you must opt-in to use the feature, which a strongly recommend to harden your workflow's security.
...
With these steps completed, the **GitHub Actions** workflows (executed from your project's repository) will be able to request ephemeral tokens to Azure. These tokens will be valid only for the duration of the job and scoped for the selected subscription and Azure RBAC Roles.
